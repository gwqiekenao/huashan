	...
	<div class="main">
		...
		<div class="main_panel">
			<div class="content">
				...
				<form action="${pageContext.request.contextPath}/project/updatePro" method="post" name="updateProForm">
				<div class="tableBox">
					<table class="table_ht">
						<tbody>
						<tr>
							<td width="20%">&nbsp;项目编号：</td>
							<td width="30%">${project.projectCode }
								<input type="hidden" value="${project.id }" name="id">
							</td>
							<td width="20%"><span class="notice">*</span>项目名称</td>
							<td width="30%">
								<input type="text" value="${project.projectName }" name="projectName">
							</td>
						</tr>
						...
						<c:forEach items="${sessionScope.fastAll}" var="fast">
							<c:if test="${fast.fastType == 'projectType'}">
														<c:choose>
															<c:when
																test="${fast.code == project.projectType }">
						...
						<c:forEach items="${sessionScope.fastAll}" var="fast">
							<c:if test="${fast.fastType == 'moneyType'}">
														<c:choose>
															<c:when
																test="${fast.code == project.moneyType }">
																<option value="${fast.code}" selected>${fast.codeName}</option>
															</c:when>
															<c:otherwise>
																<option value="${fast.code}">${fast.codeName}</option>
						...
						<tr>
							<td>合同金额(万元)</td>
							<td>
								<input id="htje" type="number" min="0" name="contractMoney" value="${project.contractMoney }">
							</td>
							<td>概算金额(万元)</td>
							<td>
								<input id="gsje" type="number" min="0" name="estimateMoney" value="${project.estimateMoney }">
							</td>
						</tr>
						<tr>
							<td>材料限额(万元)</td>
							<td>
								<input id="clje" type="number" min="0" name="materialMoney" value="${project.materialMoney }">
							</td>
							<td><span class="notice">*</span>项目经理</td>
							<td>
								<input id="xxjl" type="text" value="${project.projectManager }" name="projectManager">
							</td>
						</tr>
						<tr>
							<td><span class="notice">*</span>电话</td>
							<td>
								<input id="dh" type="text" value="${project.managerTel }" name="managerTel">
							</td>
							<td>身份证</td>
							<td><input id="dh" type="text" value="${project.managerIdCard }" name="managerIdCard"></td>
						</tr>
						
						<tr>
							<td><span class="notice">*</span>项目地址</td>
							<td colspan="3">
								<input id="xmdz" type="text" value="${project.projectAddress }" name="projectAddress">
							</td>
						</tr>
						<tr>
							<td><span class="notice">*</span>项目概述</td>
							<td colspan="3">
								<textarea name="projectInfo" id="xmgs" cols="30" rows="3">${project.projectInfo }</textarea>
							</td>
						</tr>
						</tbody>
					</table>
