...
<body>
		<div>
			<form action="userController/deleteUsers?" name="deleteUsers" method="post">
			...
			<div class="main_panel">
				<div class="content">
					...
					<div class="searchCont">
						<form action="userController/findAllUser" method="post"
							name="queryUserForm"  id="queryUserForm">
							<div class="searchCont">
								<label class="margin10" for="ygbh" >员工编号</label> 
								<input name="userCode"  type="text"
								value="${requestScope.userCode }"> 
								 								 
								 <label class="margin10" for="mc">名称</label>
							     <input name="userName" class="field" type="text"
							     value="${requestScope.userName }">  
							     
							     <label class="margin10" for="phoneNum">手机</label>  
							      <input name="userTel" class="field" type="text" onkeyup="userTel1()"
							      value="${requestScope.userTel }">  
							      							      
							      <label id="msg" style= "color:red" ></label> 
							      
								  <a class="btn clearBtn fr mr5" href="userController/findAllUser" id="clearBtn"><b></b>清空</a> 
								<a class="btn queryBtn fr mr5" href="javascript:void()" onclick="verify()"><b></b>查询</a>
							</div>
							...
							<div class="topTitle">
								...
								<div class="editCont">
									<ul class="editList">
										...
										<li><a class="addBtn" href="userController/insertUser"><b></b>新增</a></li>
							 ...
							 <div class="tableBox">
								...
								<c:forEach items="${requestScope.userList }" var="user" varStatus="status"> 
									    ...
									    <tr align="center">
										<td width="1%"><input type="checkbox" class="allCheck" name="sel" onclick="selTr(this)"/></td>
									 
										<td>${status.index+1 }</td>
										<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<a href="userController/update?id=${user.id}"><b></b>修改</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<a href="userAndRoleController/jump?id=${user.id}"><b></b>授权</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<a class="delBtn"
							href="${pageContext.request.contextPath}/userController/deleteUser?id=${user.id }"
							onclick="if (confirm('确定要删除此项数据吗？')) return true; else return false;">删除</a></td> 
										<td width="8%">${user.userCode }</td>
										<td width="8%">${user.userName }</td>
										 
										<td width="8%">${user.userTel }</td>
										<td width="8%">${user.userCountry }</td>
										<td width="8%">${user.userSex }</td>
										<td><fmt:formatDate
...
<div id="quitBox" class="pop">
	...
	function exportExcel() {		
		...
		if (flag == true) {
			...
			window.location.href='${pageContext.request.contextPath}/xitong/user_UserExcel2?expUsers='+expUsers;
			$("#selAll").prop("checked", false);
		}
