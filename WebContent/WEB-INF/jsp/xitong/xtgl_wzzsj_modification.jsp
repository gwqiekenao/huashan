	...
	<div class="main">
		...
		<div class="main_panel">
			<div class="content">
				...
				<div class="tableBox">
				<form action="${pageContext.request.contextPath }/materialUpdate" id="materialForm">
					<input type="hidden" name="id" value="${materialInfo.id }" />
					...
					<table class="table_ht">
						...
						<tr>
							...
							<td width="35%">
								<span id="wzbh" >${materialInfo.materialCode }</span>
								<input name="materialCode" type="hidden" value="${materialInfo.materialCode }">
							</td>
							<td width="15%"><span class="notice" id="yzmzmc">*</span>物资名称 </td>
							<td width="35%">
								<input id="wzmc" name="materialName" type="text" value="${materialInfo.materialName }" onblur="yzmzmc()">
							</td>
						</tr>
						<tr>
							<td><span class="notice">*</span>单位</td>
							<td>
								<select class="dropdown htlx" id="htlx"  name="materialUnit">
									<option value="${materialInfo.materialUnit }">
										<c:forEach items="${sessionScope.fastAll}" var="fast">
										<c:if test="${fast.code == materialInfo.materialUnit }">${fast.codeName}</c:if>
										</c:forEach>
									</option>
								  <c:forEach items="${sessionScope.fastAll}" var="fast">
										<c:if test="${fast.fastType == 'materialUnit' && fast.code != materialInfo.materialUnit}">
							...
							<td>
								<select class="dropdown htlx" id="wzxh" name="materialType">
									<option value="${materialInfo.materialType }">
										<c:forEach items="${sessionScope.fastAll}" var="fast">
										<c:if test="${fast.code == materialInfo.materialType }">${fast.codeName}</c:if>
										</c:forEach>
									</option>
									<c:forEach items="${sessionScope.fastAll}" var="fast">
										<c:if test="${fast.fastType == 'materialType' && fast.code != materialInfo.materialType }">
										<option value="${fast.code}">${fast.codeName}</option>
										</c:if> 
									</c:forEach>
								</select>
						...
						<tr>
							...
							<td>
								<input id="gsdj" name="estimateMoney" class="field" type="number" value="${materialInfo.estimateMoney }" onblur="yzgsdj()">
							</td>

							<td><span class="notice" id="yzbzth">*</span>标准/图号</td>
							<td>
								<input id="wzxil" name="standard" class="field" type="text" value="${materialInfo.standard }" onblur="yzbzth()">
							</td>	
						</tr>
						<tr>
							<td><span class="notice" id="yzwzmz">*</span>毛重</td>
							<td>
								<input style="display:inline-block;width:42%;float:left;" id="mz" class="field" type="number" min="0" name="grossWeight" value="${materialInfo.grossWeight }" onblur="yzwzmz()">
							</td>
							<td><span class="notice" id="yzwzjz">*</span>净重</td>
							<td>
								<input style="display:inline-block;width:42%;float:left;" id="jz" class="field" type="number" min="0" name="netWeight" value="${materialInfo.netWeight }" onblur="yzwzjz()">
							</td>
						</tr>
						<tr>
							...
							<td class="qxTd">
								<div class="input_search start">
									<div class="searchForm">
										<input type="text" name="stratTime1" class="input_time field wzyxksrq" id="wzyxksrq" value="${stratTime }">
								...
								<div class="input_search end">
									<div class="searchForm">
										<input type="text" class="input_time field wzyxjsrq" id="wzyxjsrq" name="endTime1" value="${endTime }" onblur="yzwzrq()">
										<b class="icon icon-date date"></b>
									</div>
								</div>
							</td>
						...
						<tr>
							<td><span class="notice" id="yzwzbz">*</span></td>
							<td colspan="3">
								<textarea name="materialRemark" id="beizhu" cols="30" rows="5" onblur="yzwzbz()">${materialInfo.materialRemark }</textarea>
							</td>
						</tr>
						</tbody>
					</table>
				...
				<div class="submitCont">
					<input class="returnBtn btn fr mr5" type="button" onclick="backWZSJ()" value="返回" />
					<input class="saveBtn btn fr mr5" type="button" onclick="updateMaterial()" value="提交" />
				</div>
			</div>
		</div>
	</div>
...
<script type="text/javascript"> 
	...
	function updateMaterial(){
		...
		if(yzmzmc == null || yzmzmc == ""){
			$("#yzmzmc").text("不能为空");
		}else if(yzgsdj == null || yzgsdj == ""){
			$("#yzgsdj").text("不能为空");
		}else if(yzbzth == null || yzbzth == ""){
			$("#yzbzth").text("不能为空");
		}else if(yzwzmz == null || yzwzmz == ""){
			$("#yzwzmz").text("不能为空");
		}else if(yzwzjz == null || yzwzjz == ""){
			$("#yzwzjz").text("不能为空");
		}else if(yzwzbz == null || yzwzbz == ""){
			$("#yzwzbz").text("不能为空");
		}else if(wzyxksrq == null || wzyxksrq == "" || wzyxjsrq == null || wzyxjsrq == ""){
			$("#yzwzrq").text("不能为空");
		}else if(sd > ed){
			$("#yzwzrq").text("日期有误");
		}else{
			 $.post("${pageContext.request.contextPath}/materialUpdate",$("#materialForm").serialize(),function(data){
			alert("修改成功");
