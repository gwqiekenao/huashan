				...
				<div class="tableBox">
					<table class="table_ht">
						...
						<tr>
							...
							<td width="35%">${supplier.supplierCode}</td>
							<td width="15%" >供应商名称：</td>
							<td width="35%">${supplier.supplieName}</td>
						</tr>
						<tr>
							<td>供应商类型：</td>
							<td><c:forEach items="${sessionScope.fastAll}" var="fast">
									<c:if test="${fast.fastType == 'supplierType' }">
											<c:if test="${fast.code == supplier.supplierType }">${fast.codeName}</c:if>
											</c:if>
										</c:forEach></td>
							<td>营业执照注册号：</td>
						...
						<tr>
							<td>组织机构代码：</td>
							<td>${supplier.organiztion}</td>
							<td>税务登记证(国税)：</td>
							<td>${supplier.stateTax}</td>
						</tr>
						<tr>
							<td>税务登记证(地税)：</td>
							<td>${supplier.landTax}</td>
						</tr>
						</tbody>
					</table>
				</div>
