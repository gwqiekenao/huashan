	...
	calendar.prototype={
		...
		bindData:function(currTime,form){
			...
			daysCon.on('click',function(e){
				...
				if(_target && !e.target.className.indexOf('disabled')>-1){
					...
					if(nextInput && nextInput.className.indexOf('widget_timer')>-1){
						...
						$(nextInput).click();
					}else if(prevInput&& !prevInput.className.indexOf('widget_timer')>-1){
						prevInput.click();
					}
			...
			form.find('.monSelect').on('click',function(e){
				...
				box.html('<div class="yearBox"></div><div class="monthBox"></div><div class="tc"><input type="button" value="确定"></div>');
				for(var i=self.options.beginYear;i<=self.options.endYear;i++){
					df+='<span>'+i+'</span>';
				}
				box.find('.yearBox').html(df);
				box.find('.monthBox').html('<span>1月</span><span>2月</span><span>3月</span><span>4月</span><span>5月</span><span>6月</span><span>7月</span><span>8月</span><span>9月</span><span>10月</span><span>11月</span><span>12月</span>');
		...
		show:function(target){
			...
			if(target.value!=target.options.tip && target.value.length>0){
				var tmpYear=parseInt(target.value.substring(0,4)),
					tmpDay=parseInt(target.value.substring(8,10)),
		...
		format2Day:function(date,n){
			...
			if(typeof n==='number'){
				var d=new Date(tmpDate[0],tmpDate[1]-1,tmpDate[2],23,59,59);
